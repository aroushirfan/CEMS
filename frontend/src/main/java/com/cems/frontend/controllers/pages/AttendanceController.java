...
/**
 * Controller for the attendance view, which displays the list
 * of attendees for a specific event. It allows users to search, sort,
 * and view attendance records, as well as navigate back to the event details.
 */
public class AttendanceController {
  ...
  private final ObservableList<Attendance> attendanceModelObservable =
   ...
   *
   * @param event The event for which to load attendance records.
   */
  ...
  /**
   * Initializes the attendance view by setting up the
   * table view, search functionality, and localization.
   * This method is called automatically by the JavaFX framework
   * after the FXML components are loaded.
   */
  @FXML
  public void initialize() {
    resourceBundle = localeService.getBundle(Paths.ATTENDANCE_VIEW);
    attendanceTableView.setPlaceholder(new Label(
        resourceBundle.getString("attendance.no_attendance_data")));
  ...
  private void attendanceSearchFilter() {
    ...
    FilteredList<Attendance> filteredData = new FilteredList<>(
        attendanceModelObservable, b -> true);
    searchTextField.textProperty().addListener((
      ...
      filteredData.setPredicate(attendee -> {
        ...
        return attendee.getEmail().toLowerCase().contains(search)
            || attendee.getName().toLowerCase().contains(search);
      });
    });
    ...
    checkInTimeColumn.setCellFactory(col -> new TableCell<>() {
      ...
      protected void updateItem(Instant time, boolean empty) {
        ...
        if (empty) {
          ...
          setText(time == null ? resourceBundle.getString("attendance.check_in_time_unavailable")
              : time.atZone(ZoneId.systemDefault()).format(localeService.time(FormatStyle.MEDIUM)));
        }
      }
    });
  }
  ...
  private void updateAttendanceRecords() {
    ...
    long attendancePresentCount = attendanceModelObservable.stream()
        .filter(attendance -> attendance.getStatus().equals("PRESENT")).count();
