...
public class ApiEventService implements IEventService {
     ...
     */
    public ApiEventService() {this.client = LocalHttpClientHelper.getClient();}
     ...
     * @throws Exception if serialization, transport, or non-handled HTTP errors occur
     */
    @Override
     ...
     * @throws Exception if serialization, transport, or creation response handling fails
     */
    @Override
    public Event createEvent(EventDto.EventRequestDTO data) throws Exception {
        final String json = mapper.writeValueAsString(data);
        final HttpRequest request = LocalHttpClientHelper.buildRequest(BASE_URL).authorization(authService.getToken()).post(json);
        final HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
      ...
      * @throws Exception if serialization, transport, or update response handling fails
      */
    @Override
    public Event updateEvent(String id, EventDto.EventRequestDTO data) throws Exception {
        final String json = mapper.writeValueAsString(data);
        final HttpRequest request = LocalHttpClientHelper.buildRequest(BASE_URL + "/" + id).authorization(authService.getToken()).post(json);
      ...
      *
      * @param id event identifier
      * @throws Exception if transport fails or backend does not return a successful delete status
      */
    @Override
    public void deleteEvent(String id) throws Exception {
        final String url  = String.format("%s/%s", BASE_URL, id);
        final HttpRequest request = LocalHttpClientHelper.buildRequest(url).authorization(authService.getToken()).delete();
        final HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        if (response.statusCode() != HttpStatus.NO_CONTENT.code && response.statusCode() != HttpStatus.OK.code) {
      ...
      * @param id event identifier
      * @return event model
      * @throws Exception if transport fails or event retrieval is unsuccessful
      */
    @Override
    public Event getEventById(String id) throws Exception {
      ...
      * @throws Exception if transport fails or backend approval response is unsuccessful
      */
    @Override
    public Event approveEvent(String id) throws Exception {
        final String url = String.format("%s/events/%s/approve", BASE_URL, id);
        final HttpRequest request = LocalHttpClientHelper.buildRequest(url).authorization(authService.getToken()).put(null);
